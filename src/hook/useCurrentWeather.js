...
const getCurrentWeather = async (data) => {
  try {
    ...
    if (data.queryKey[3]) {
      url = `https://api.openweathermap.org/data/2.5/weather?q=${data.queryKey[3]}&appid=${API_KEY}&units=metric&lang=KR`;
    }
...
export const useCurrentWeather = (lat, lon, searchCity) => {
  return useQuery({
    queryKey: ['currentWeather', lat, lon, searchCity],
